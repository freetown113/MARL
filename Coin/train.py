...
def train():
    ...
    net_2 = model.Network(env.observation_space.shape,
                                        env.action_space.n, 1).to(device)
    ...
    for i in count():
        ...
        if len(buf) >= REPLAY_INITIAL:
            ...
            states, act_1, act_2, rew_1, rew_2, dones, n_states, indices, weights = agnt_1.get_batch_data()
            loss_1, priorities_1, indices_1 = agnt_1.train(states, act_1, rew_1, dones, n_states, indices, weights)
            loss_2, priorities_2, indices_2 = agnt_2.train(states, act_2, rew_2, dones, n_states, indices, weights)
